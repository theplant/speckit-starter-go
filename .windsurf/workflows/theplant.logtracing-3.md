...
### Recommended Span Content

To make spans useful for error troubleshooting and debugging, include the following information:

#### 1. Core Identifiers (MUST)

Business entity IDs and critical state information that uniquely identify the operation context:
...
**Why**: Without identifiers, you cannot trace which specific entity failed or correlate errors across services.

#### 2. Count Metrics (RECOMMENDED)

Quantitative measures that indicate operation scale and results:
...
**Why**: Helps identify partial failures, performance degradation, and unexpected data volumes.

#### 3. Pagination Info (MUST for paginated queries)

Cursor and pagination state for list operations:
...
**Why**: Essential for debugging pagination issues, cursor inconsistencies, and data completeness problems.

#### 4. Performance Metrics (RECOMMENDED for critical/long operations)

Timing and performance indicators for time-sensitive or potentially slow operations:

```go
ctx, _ = logtracing.StartSpan(ctx, "service.ProcessBatch")
defer func() {
    logtracing.AppendSpanKVs(ctx,
        "batch_size", batchSize,
        "processing_rate_per_sec", float64(processed)/duration.Seconds(),
        "db_query_count", queryCount,
        "external_api_calls", apiCallCount,
    )
    logtracing.EndSpan(ctx, xerr)
}()
```

**Why**: Identifies performance bottlenecks, helps set SLA alerts, and tracks degradation over time.

#### 5. Error Context (MUST on failure)

Detailed context when operations fail:
...
**Why**: Provides immediate context for failures without needing to reproduce the issue.

### Common Metric Keys

| Category        | Key Examples                                                                            |
| --------------- | --------------------------------------------------------------------------------------- |
| **Identity**    | `user_id`, `member_id`, `campaign_id`, `order_id`, `transaction_id`                     |
| **Count**       | `requested_count`, `fetched_count`, `success_count`, `failed_count`, `total_items`      |
| **Pagination**  | `limit`, `after_cursor`, `before_cursor`, `has_next`, `has_prev`, `total_count`         |
| **Performance** | `batch_size`, `processing_rate_per_sec`, `db_query_count`, `api_call_count`             |
| **Error**       | `error_stage`, `invalid_field`, `retry_attempt`, `external_service`, `grpc_status_code` |
| **State**       | `status`, `operation_type`, `filter_applied`, `sort_direction`                          |
