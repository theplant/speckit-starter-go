...
**Bump Rationale**: Added handlers package for server setup with ErrorHandler. ErrorResponse defined in OpenAPI.

**Modified Principles**:
- "API Data Structures (Schema-First Design)" - Services implement ogen Handler interface
- "Service Layer Architecture" - Services implement ogen Handler, handlers package for server setup
...
- `handlers/` package provides `NewServer()` wrapper with ErrorHandler configuration
...
- ✅ plan-template.md - Updated for handlers package
- ✅ spec-template.md - Updated for handlers package
- ✅ tasks-template.md - Updated for handlers package
-->
...
**Server Setup** (via handlers package with ErrorHandler):
```go
// handlers/server.go
package handlers
...
func main() {
    ...
    server, err := handlers.NewServer(service)  // Uses ErrorHandler
    if err != nil {
        log.Fatal(err)
    }
...
- **CRITICAL**: `handlers/` package MUST provide `NewServer()` wrapper with ErrorHandler configuration
- Services are reusable Go packages with OpenAPI-defined interfaces
...
- `cmd/main.go` MUST use `handlers.NewServer()` (NOT `api.NewServer()` directly)
- If `cmd/main.go` needs functionality from `internal/`, that code MUST be promoted to a public service

**Architecture**: `HTTP (ogen server with ErrorHandler) → Service (implements Handler) → GORM Model → Database`

**Rationale**: Services implement ogen Handler interface directly, making them reusable Go packages. The `handlers/` package encapsulates server setup with ErrorHandler to map service errors to user-friendly HTTP responses. External apps can import services and use them directly with OpenAPI-defined interfaces. ogen handles HTTP routing, request parsing, and response serialization. Services in `internal/` cannot be imported externally (Go visibility rules).
...
**Version**: 3.2.0 | **Ratified**: 2025-11-20 | **Last Amended**: 2025-12-25
